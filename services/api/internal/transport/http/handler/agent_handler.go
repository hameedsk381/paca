...
func (h *AgentHandler) StoreMemory(c *gin.Context) {
	...
	presenter.Created(c, dto.AgentMemoryFromEntity(mem))
}
...
func (h *AgentHandler) SearchMemories(c *gin.Context) {
	...
	resp := make([]dto.AgentMemoryResponse, 0, len(mems))
	for _, m := range mems {
		resp = append(resp, dto.AgentMemoryFromEntity(m))
	}
	presenter.OK(c, resp)
}
...
func (h *AgentHandler) CreateApprovalRequest(c *gin.Context) {
	...
	presenter.Created(c, dto.ApprovalRequestFromEntity(ar))
}
...
func (h *AgentHandler) ListApprovalRequests(c *gin.Context) {
	...
	resp := make([]dto.ApprovalRequestResponse, 0, len(reqs))
	for _, r := range reqs {
		resp = append(resp, dto.ApprovalRequestFromEntity(r))
	}
	presenter.OK(c, resp)
}
...
func (h *AgentHandler) ResolveApprovalRequest(c *gin.Context) {
	...
	presenter.OK(c, dto.ApprovalRequestFromEntity(ar))
}
